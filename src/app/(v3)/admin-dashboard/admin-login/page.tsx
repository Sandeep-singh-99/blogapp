...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await fetch("/api/adminDashboardLogin", {
        method: "POST",
        headers: {
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            ...
            <input
              ...
              className="mt-1 block w-full px-3 py-2 border text-black dark:text-white border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
            />
          </div>
          <div className="mb-4">
